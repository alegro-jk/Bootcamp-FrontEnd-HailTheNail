...
class EmpViewPromos extends Component {
    ...
    constructor (props) {
        ...
        this.state = {
            ...
            promos: [
                { id: 1, name: "Christmas Promo", month: "December", year: 2021 },
    ...
    renderTableData() {
        return this.state.promos.map((promos, index) => {
           const { id, name, month, year } = promos
           return (
              <tr key={id}>
                 <td>{id}</td>
                 <td>{name}</td>
     ...
     renderTableHeader() {
        let header = Object.keys(this.state.promos[0])
